...
void Keypad::initialize(char *userKeymap, Uint8 *row, Uint8 *col, Uint8 numRows, Uint8 numCols)
{
	rowPins = row;
    columnPins = col;
    size.rows = numRows;
...
KeyState Keypad::getKeyState() {
	...
	case IDLE:
		...
		if( buttons == KEYPAD_CLOSED && (millis() - Timer) > debounceTime) {
